...
| 随包主题 seed | 🟠 | `internal/themebundle/*`、`internal/bundledtheme/*`、`web/install/install.go` | 误在存量实例上 seed/改写主题；半解压残留；覆盖用户已删除的主题 | **只在全新安装（零用户）路径调用**；`data/theme/<short>` 已存在时绝不覆盖；解压走临时目录 + 原子 rename；任何失败都不得让安装失败（仅 warning，保持 `default`） |
