...
| 已移除的远控边界 | 🔴 | `web/router/router.go`、`web/agent/capabilities.go`、`web/api/client/report_v2.go`、`scripts/check_remote_control_removed.py` | 后续改动误恢复命令、终端或远程文件能力 | 不得恢复实现或 RPC；旧路径保持 410；远控 capability 必须过滤；CI 静态守卫必须通过 |
...
| 本地管理上传 | 🟠 | `web/upload/*`、`web/api/admin/archive_upload.go` | 路径穿越、越权写入、大小/分片校验缺陷 | 校验路径规范化、管理员权限与大小限制 |
...
| 公开主题 vs 后台 SW | 🟠 | `web/public/public.go`、`komari-web-stable` `vite.config.ts` | `theme=next` 时根作用域 Workbox 把 Next 的 `index.html` 当成 `/admin` fallback；curl 测不到 SW | 后台/恢复路径必须 force default、剥离 SW 注册；PWA 网络绕过必须覆盖 `/admin`、`/terminal` tombstone 与 `/manage`；改动要跑 `e2e/` Playwright |
