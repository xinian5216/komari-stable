...
- 安全收缩：移除服务端远程命令、Web 终端和 Agent 远程文件管理实现；旧 HTTP 路径固定返回
  `410 Gone`，旧 RPC 方法不再注册。
- 兼容边界：保留 v2 方法常量和任务表，旧 Agent 仍可连接/上报；旧任务/文件结果确认后丢弃，
  启动时仅把遗留未完成任务标记为取消。
- 回归防护：新增路由/RPC/能力/数据库测试与 `scripts/check_remote_control_removed.py` CI 静态守卫。
